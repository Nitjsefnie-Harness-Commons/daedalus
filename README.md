...
| `daedalus_mcp/server.py` | MCP composition point — registers tool groups and runs the listener (`python3 -m daedalus_mcp.server` from the repository root runs it standalone) |
| `daedalus_mcp/auth.py` | MCP authentication middleware and JSON authority-carrier validation |
| `daedalus_mcp/request_guard.py` | MCP pre-body authentication, size refusal, and bounded refusal draining |
| `daedalus_mcp/transport.py` | MCP HTTP sessions bound to the local bridge |
| `daedalus_mcp/tools_*.py` | Seven grouped MCP tool modules for the extension command surface |
