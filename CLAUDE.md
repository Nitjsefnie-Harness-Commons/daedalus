...
- `POST /segment?job=X&seg=N&total=T&sig=S` — raw binary HLS segment storage (`$DAEDALUS_DIR/segments/{job}/{seg:06d}.ts`); `sig` is the job-scoped capability from `POST /segment-job`, never the bridge token. Everything the route authorizes on is in the query string, so the capability, the parameter shapes and the index bound are all settled before the body is read: a refused request is answered without buffering the segment it named.
