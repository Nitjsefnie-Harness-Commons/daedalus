...
- `content.js` (ISOLATED world): message relay between page and background. On load it asks the background to replay hotfixes (`replayHotfixes`); the background selects the eligible fixes from the extension-wide `daedalus-hotfixes` record and runs each through the eval routing below, so a page whose CSP forbids dynamic compilation is reached over CDP. Hotfix source is page-observable by design — it executes in the page — but is no longer posted into it.
...
Repeated authority carriers are rejected before any value is selected, including equal or blank repeats: `token` in a query or JSON body, a repeated `Authorization` or `X-Daedalus-Segment-Sig` header, and `job` / `sig` on segment-capability routes.

Authenticated `GET` routes resolve `Authorization: Bearer <token>` first and fall back to `?token=`; a header and a query token in one request must be the same one, or the answer is 400 `conflicting token`. `POST /segment` and `GET /segment-status` take their job capability from `X-Daedalus-Segment-Sig` on the same rules, falling back to `sig` in the query; a repeated capability header answers 400 `duplicate segment capability header` before either value is selected. Every in-repo client sends the header form, so no reusable credential is written into a request target.

Every body-reading verb (`POST`, `PUT`, `DELETE`) settles credentials before the body is read, from `Authorization: Bearer <token>`. A body `token` is still accepted, but only for a declared body within `DAEDALUS_MAX_UNAUTHENTICATED_BODY` (default `64 * 1024` bytes) — a larger body with no header is answered 401 unread, since a body token cannot be checked without reading the body. Carrying both is allowed only when they are the same token; differing ones answer 400 `conflicting token`, and a repeated `Authorization` header answers 400 before either value is selected. `POST /segment` is unaffected: it authorizes on the query string and the request headers, and never reaches this gate.
...
- `POST /result` — extension posts results here. The server exposes a nonempty command `_did` as `deliveryId`, assigns a fresh `resultGeneration`, and writes the shared result slots while holding the result lock. A delivery id already stored is answered `{ok, duplicate: true}` with both slots and the dashboard stream left untouched, so a retried result never republishes over a newer one; the record of stored delivery ids is in-memory and bounded at 4096.
