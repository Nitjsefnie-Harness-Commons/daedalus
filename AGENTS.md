...
- Every route module binds no configuration of its own; three cannot yet be imported without it, because the stores they import read `config`: `result_routes` via `result_store`, and `segment_routes` / `segment_jobs` via `segment_store` (issue 484). Their own directory arguments stay parameters, so they must name the configured roots until those stores take theirs.
</architecture>
...
- `POST /result` — where the extension posts results. The server exposes a nonempty command `_did` as `deliveryId`, assigns a fresh `resultGeneration`, and writes the shared result slots while holding the result lock. For a delivery id, one per-target lock is acquired first and owns the delivery write, acceptance stamp, eviction, and cross-copy cleanup; the result lock is nested only after it. A nonempty delivery id is also retained at `results/deliveries/<target>/<deliveryId>.json`. A delivery id already stored is answered `{ok, duplicate: true}` with both slots and the dashboard stream left untouched, so a retried result never republishes over a newer one; the record of stored delivery ids is in-memory and bounded at 4096.
