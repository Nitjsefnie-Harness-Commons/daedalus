...
- Keep comments concise, and aim for at most a 15% comment ratio (bytes of `#` comments and docstrings over all bytes, whitespace included on both sides, tree-wide). Write only what a reader cannot recover from the code itself; never restate what the next line already says. Go past 15% only where the reasoning genuinely needs it.
