...
def _server_module(tmp, name):
    """Import server.py for a unit-level check, the way the _log_safe test does.

    It reads DAEDALUS_DIR and DAEDALUS_PORT at import and refuses to load
    without them, so both are supplied and then put back.
    """
    env = {'DAEDALUS_DIR': str(tmp), 'DAEDALUS_PORT': '8081'}
    saved = {key: os.environ.get(key) for key in env}
    os.environ.update(env)
    ...
    if added_root:
        sys.path.insert(0, root)
    try:
        return _util.load(_util.ROOT / 'server.py', name)
    finally:
        if added_root:
            sys.path.remove(root)
        for key, value in saved.items():
...
def test_a_queued_command_can_be_claimed_only_once(tmp):
    """Two consumers covering one queue: exactly one may deliver each file.

    An extension stream drains every per-tab queue and a specific-tab stream
    drains its own, so both can discover the same file. Discovery is not
    ownership — before the claim, both read it, both wrote the frame, and the
    command went out twice under one delivery id.
    """
    server = _server_module(tmp, 'server_claim')
    qdir = Path(tmp) / 'queue'
    qdir.mkdir()
    queued = qdir / '0000000000001_000001.json'
    queued.write_text('{"id":"once"}', encoding='utf-8')

    first = server._claim_command(queued)
    second = server._claim_command(queued)
    assert first is not None, 'the first consumer could not claim the file'
    assert second is None, ('the second consumer claimed it too: '
                            f'{first} and {second}')
    assert first.is_file(), first
    assert not queued.exists(), 'the claimed file kept its visible name'
    # A claimed name starts with a dot, which every queue scan skips, so no
    # later pass picks it up either.
    assert first.name.startswith('.'), first.name


def test_a_claim_that_cannot_be_delivered_goes_back(tmp):
    """A claim is only good while its holder can deliver it.

    The socket write happens after the claim, so a failed write has to put the
    file back under its visible name; leaving it claimed would lose the
    command to a name no scan looks at.
    """
    server = _server_module(tmp, 'server_claim_back')
    qdir = Path(tmp) / 'queue'
    qdir.mkdir()
    queued = qdir / '0000000000002_000002.json'
    queued.write_text('{"id":"returned"}', encoding='utf-8')

    claimed = server._claim_command(queued)
    assert claimed is not None
    claimed.rename(queued)  # what the delivery path does when the write fails
    assert queued.is_file(), 'the command did not come back'
    assert server._claim_command(queued) is not None, (
        'a returned command could not be claimed again')


def test_two_streams_covering_one_queue_deliver_a_command_once(tmp):
    """The claim is wired into the drain, not merely available to it.

    An extension stream drains every per-tab queue and a specific-tab stream
    drains its own, so one command in that tab's queue is visible to both.
    Exactly one of them may carry it: two frames for one delivery id is the
    command running twice in the browser.
    """
    # The unlink race is microseconds wide, so both consumers have to be held
    # past the read for the interleaving to happen at all: without this the
    # loser simply finds the file gone and the test passes against unfixed
    # code, which is what the first version of it did.
    ...
    (fault_dir / 'sitecustomize.py').write_text(
        ...
        '    if path.parent.parent.name == "commands" and path.suffix == ".json":\n'
        '        time.sleep(0.75)\n'
        '    return text\n'
    ...
    with _util.bridge(tmp, env={'PYTHONPATH': str(fault_dir)},
        ...
        ext_conn, ext_response = _stream_response(base, TOK, tab='extension')
        tab_conn, tab_response = _stream_response(base, TOK, tab='dup')
        try:
            assert ext_response.status == 200, ext_response.status
            assert tab_response.status == 200, tab_response.status
            status, _ = _put_command(
                base, {'token': TOK, 'tab': 'dup', 'id': 'once', 'code': '1'})
            assert status == 200, status
            delivered = []
            for label, response in (('extension', ext_response),
                                    ('dup', tab_response)):
                try:
                    frame = _next_stream_data(response, timeout=6)
                except AssertionError:
                    continue  # this stream lost the claim, which is the point
                delivered.append((label, frame.get('id')))
            assert len(delivered) == 1, (
                f'one queued command reached {len(delivered)} streams: '
                f'{delivered}; the bridge said: {"".join(served)!r}')
            assert delivered[0][1] == 'once', delivered
        finally:
            ext_response.close()
            ext_conn.close()
            tab_response.close()
