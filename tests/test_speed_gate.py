#!/usr/bin/env python3
"""The speed gate's shape: an aggregate dependency, one name, one verdict.

The pins here hold three things together: the measuring cells start after the
tests workflow's aggregate job instead of polling it, the `timed` job keeps a
static check name and names each cell's artifact after its generated group,
and the final job keeps the check name and reads the cells' own verdict
records. The matrix CONTENTS are generated now, so the partition belongs to
the planner (`test_timed_planner.py`), not here. The steps' runtime behaviour
is executed here; other invariants over the merged workflow stay in
...
def test_the_timed_job_keeps_a_static_name_and_per_group_artifacts(tmp):
    """The check name is static; each cell's reports land in its own group.

    A skipped matrix job still creates one check run, named after the job's
    `name:` verbatim, so an expression there leaks the raw template into that
    name. The matrix VALUES are now generated by the planner, but the name
    is not, and each cell's durations land in an artifact named after the
    group the planner gave it.
    """
    del tmp
    workflow = _tests_yml()
    timed = complete_job_mapping(workflow, 'timed')
...
def test_each_cell_measures_its_reference_unit_and_writes_it(tmp):
    """The producer half of the reference-artifact contract is pinned.

    `refresh_timings.py` reads each cell's `reference.json` as the unit its
    weights are counted in, and that consumer is tested against a hand-built
    run tree. Without a pin on the step that PRODUCES the file, deleting it
    leaves every check green and the refresher silently receiving no unit.
    """
    del tmp
    workflow = _tests_yml()
    script = workflow_script(workflow, 'timed', 'Measure the reference')
    assert 'reference_workload.py' in script, script
    assert 'reports/reference.json' in script, script
