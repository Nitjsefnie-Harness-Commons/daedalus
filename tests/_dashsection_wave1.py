...
# `api.js` sleeps `POLL_CADENCE_MS` between result attempts, and the
...
# only SHORTEN the count, never lengthen it.
#
# So the one host-independent statement about a leg count is an upper
# bound, and that is all a case asserts. An exact count is a wall-clock
# margin wearing a number's clothes, and a one-sided band is a margin with
# a derivation attached -- `controls/2026-09-08-a-bound-is-derived-from-
# the-domain-or-it-is-a-sample`, whose case is a slack chosen to feel
