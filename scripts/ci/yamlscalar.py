...
The complete decoders read a bare-empty mapping value (`key:` with nothing
under it) as `None`, matching `yaml.safe_load` where `yaml.BaseLoader` gives
`''`; `key: ''` stays `''`. Spelled nulls read as their spelling, and the
step reader still refuses an empty field.
"""
import re
